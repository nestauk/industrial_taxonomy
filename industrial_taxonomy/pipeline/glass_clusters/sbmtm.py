...
class sbmtm:
    ...
    def load_graph(self, filename="graph.gt.gz"):
        ...
        self.words = [
            self.g.vp["name"][v] for v in self.g.vertices() if self.g.vp["kind"][v] == 1
        ]
        self.documents = [
            self.g.vp["name"][v] for v in self.g.vertices() if self.g.vp["kind"][v] == 0
        ]
